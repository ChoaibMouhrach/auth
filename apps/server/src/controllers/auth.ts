...
export const authController = new Hono<AppContext>()
  .post(
    ...
    async (context) => {
      ...
      const { code } = await services.auth.signIn({
        email: body.email,
  ...
  .post(
    ...
    async (context) => {
      ...
      const { code } = await services.auth.resetPassword({
        token,
        clientId,
        redirectUrl,
        password: body.password,
  ...
  .get(
    ...
    async (context) => {
      ...
      const { code } = await services.auth.confirmEmail({
        token,
        clientId,
        redirectUrl,
      });
      ...
      url.searchParams.set("code", code.data.code);
  ...
  .post(
    "/exchange-code",
    zValidator("json", z.object({ code: z.uuid(), secret: z.uuid() })),
    async (context) => {
      const { services } = context.get("context");
      const body = context.req.valid("json");

      const { session } = await services.auth.exchangeCode({
        code: body.code,
        secret: body.secret,
      });
